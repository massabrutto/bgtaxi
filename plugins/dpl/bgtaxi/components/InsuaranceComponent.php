...
use Dpl\Bgtaxi\Controllers\Insuarences;
use Dpl\Bgtaxi\Models\Insurance;
use Illuminate\Support\Facades\Cache;

class InsuaranceComponent extends ComponentBase
{
    protected $sSlug;
    ...
    public function getItem()
    {
        $arCachedItem = Cache::remember('arCachedItem' . $this->sSlug . __CLASS__ . __METHOD__, Carbon::now()->addMonth(), function(){
            $obItem = Insurance::active()->where('slug', $this->sSlug)->first();
            if (empty($obItem)) {
                return;
            }
            $arGallery = [];
