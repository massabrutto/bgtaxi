...
app.fn.sendEmailPopup = function(elem) {
    ...
    if(location.search.split('popup=')[1] == 'show') {
        $.magnificPopup.open({
            items: {
                src: '<div class="white-popup _email"><h4 class="white-popup__ttl">Message sent! Thank you for your request!</h4></div>',
                type: 'inline'
            },
            callbacks: {
                open: function() {
...
app.fn.FixedMenu.prototype.init = function() {
    ...
    if(self.$window.scrollTop() >= (self.fixedMenuTop - self.headHeight) - (self.headHeight / 1.5)) {
        self.$fixedMenu.addClass('_fixed');
    } else {
        self.$fixedMenu.removeClass('_fixed');
    }

    if(self.$window.scrollTop() >= (self.fixedMenuTop + app.$contentHeight) - self.fixedMenuHeight - 250) {
